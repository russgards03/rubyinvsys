...
class Receive{
	...
	{
		$sql = "DELETE FROM tbl_receive WHERE rec_id = :id";
	...
	//Makes a list of all data from the receive table
	...
	//New product/item to be received by the business (may it be from outer or inner source)
	...
	//Saves product/item into the receive table
	...
	//Saves product/item and places it in the inventory
	public function save_to_inventory($id){
		...
		$stmt = $this->conn->prepare("INSERT INTO tbl_inventory(rec_id, product_id, product_qty) VALUES (?,?,?)");
		try {
			$this->conn->beginTransaction();
			foreach ($data as $row){
				extract($row);
